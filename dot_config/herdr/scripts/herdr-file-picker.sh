#!/usr/bin/env bash
# fzf-based file/dir picker for a herdr pane.
# Invoked from herdr via `[[keys.command]]` (type = "pane"), inheriting
# HERDR_ACTIVE_PANE_ID and HERDR_ACTIVE_PANE_CWD from the calling pane.
# Detects the active pane's AI agent via `herdr pane get` and, when found,
# prefixes selected paths with `@` before sending them back with
# `herdr pane send-text`; otherwise paths are shell-escaped.
# Requires `grep-preview` on PATH (installed by the Makefile).
set -euo pipefail
...
main() {
  if [[ -z ${HERDR_SOCKET_PATH-} ]]; then
    echo "Error: This script must be run inside a herdr session." >&2
    exit 1
  fi
  ...
  pane_dir="${HERDR_ACTIVE_PANE_CWD-}"
  if [[ -z $pane_id || -z $pane_dir ]]; then
    echo "Error: HERDR_ACTIVE_PANE_ID / HERDR_ACTIVE_PANE_CWD are not set." >&2
    echo "       (This picker is meant to run via [[keys.command]] in config.toml)" >&2
    exit 1
  fi
  ...
  case "$agent" in
    claude | codex | gemini) at_prefix_mode=true ;;
  esac
  ...
  if $dir_only; then
    ...
    herdr pane send-text "$pane_id" "$output_str"
    exit 0
  fi
  ...
  if $select_directories; then
    ...
    fd_flags="${HERDR_FILE_PICKER_FD_FLAGS:--H --follow --type d --exclude .git}"
    ...
    fd_flags="${HERDR_FILE_PICKER_FD_FLAGS:--H --follow --type f --type d --exclude .git --exclude .DS_Store}"
  ...
  # --- Send to Herdr ---
  herdr pane send-text "$pane_id" "$files_oneline"
}
